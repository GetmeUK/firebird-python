...
class Client:
    ...
    def __call__(self,
        ...
        # Build headers
        headers = {'X-Firebird-APIKey': self._api_key}
        ...
        if 'X-Firebird-RateLimit-Limit' in r.headers:
            self._rate_limit = int(r.headers['X-Firebird-RateLimit-Limit'])
            self._rate_limit_reset \
                    = float(r.headers['X-Firebird-RateLimit-Reset'])
            self._rate_limit_remaining \
                    = int(r.headers['X-Firebird-RateLimit-Remaining'])
        ...
        error_cls = exceptions.FirebirdException.get_class_by_status_code(
            r.status_code
        )
